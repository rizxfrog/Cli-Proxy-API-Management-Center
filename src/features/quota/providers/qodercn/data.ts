/**
 * Qoder CN credits data layer.
 * React-free / SCSS-free -- consumed directly by tests.
 *
 * The ledger comes from the proxy's dedicated /qoder-cn-quota endpoint, which
 * resolves the credential, refreshes a stale access token once, and merges
 * GET /api/v2/quota/usage (the credit buckets) with GET /api/v3/user/status (the
 * plan tier and reset instant).
 *
...
import { isQoderCNFile, isDisabledAuthFile } from '@/utils/quota';
 ...
 */
export function buildQoderCNQuotaRows(payload: QoderCNQuotaPayload, t: TFunction): QoderCNQuotaRow[] {
...
const fetchQoderCNQuota = async (file: AuthFileItem, t: TFunction): Promise<QoderCNQuotaData> => {
  ...
  try {
    payload = await qoderCnQuotaApi.fetchQuota(authIndex);
  } catch (error: unknown) {
    ...
    const message = error instanceof Error && error.message ? error.message : t('qodercn_quota.quota_error');
...
export const QODERCN_CONFIG: QuotaProviderData<QoderCNQuotaState, QoderCNQuotaData> = {
  ...
  buildLoadingState: () => ({ status: 'loading', rows: [], plan: null, unit: null, exhausted: false }),
